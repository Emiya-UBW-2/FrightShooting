...
void OptionWindow::SetTab(void) noexcept {
	...
	this->m_DrawUI->AddChild(ChildBase.c_str(), ("data/UI/Option/Tab" + std::to_string(this->m_NowSelectTab + 1) + ".json").c_str(), Param);
	...
	UpdateColumnStr();
	this->m_DrawUI->Get(this->m_TabButton[static_cast<size_t>(this->m_NowSelectTab)]).SetActive(true);
}
void OptionWindow::EndTab(void) noexcept {
	this->m_DrawUI->DeleteChild(("OptionUI/Child" + std::to_string(this->m_NowSelectTab + 1)).c_str());
	this->m_DrawUI->Get(this->m_TabButton[static_cast<size_t>(this->m_NowSelectTab)]).SetActive(false);
	this->m_NowTabMax = 0;
}
void OptionWindow::Init(void) noexcept {
	...
	for (auto& t : this->m_TabButton) {
		size_t index = static_cast<size_t>(&t - &this->m_TabButton.front());
		t = this->m_DrawUI->GetID(("OptionUI/Tab" + std::to_string(index + 1)).c_str());
		this->m_DrawUI->Get(t).SetActive(false);
	}
	this->m_NowSelectTab = 0;
...
void OptionWindow::Update(void) noexcept {
	...
	if (this->m_DrawUI->Get(this->m_UIBase).IsActive()) {
		...
		for (auto& t : this->m_TabButton) {
			size_t index = static_cast<size_t>(&t - &this->m_TabButton.front());
			if (this->m_DrawUI->Get(t).IsSelectButton() && KeyMngr->GetMenuKeyTrigger(Util::EnumMenu::Diside)) {
				Sound::SoundPool::Instance()->Get(Sound::SoundType::SE, OKID)->Play(DX_PLAYTYPE_BACK, TRUE);
			}
			if (this->m_DrawUI->Get(t).IsSelectButton() && KeyMngr->GetMenuKeyReleaseTrigger(Util::EnumMenu::Diside)) {
				if (this->m_NowSelectTab != static_cast<int>(index)) {
					EndTab();
					this->m_NowSelectTab = static_cast<int>(index);
					SetTab();
				}
			}
		}
